...
const App: React.FC = () => {
  ...
  if (!isAuthenticated) {
    return <LoginView onLogin={handleLogin} existingUser={user} />;
  }
    ...
    <HashRouter>
      <div className="h-screen flex flex-col lg:flex-row overflow-hidden bg-gray-50">
        ...
        <nav className="hidden lg:flex bg-slate-900 text-white w-64 flex-shrink-0 flex-col shadow-xl z-20 no-print">
          ...
          <div className="p-4 mt-auto border-t border-slate-800">
            <div className="flex items-center gap-3 mb-4 px-2">
              <div className="bg-blue-600 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold">
        ...
        <div className="flex-grow flex flex-col h-full overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex bg-gray-100 rounded-lg p-1 scale-90 lg:scale-100 origin-left">
                <button 
                  ...
                  className={`px-3 py-1 rounded-md text-xs font-bold transition-all ${activeCurrency === Currency.USD ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}
                >
                  $ USD
                </button>
                <button 
                  onClick={() => setActiveCurrency(Currency.HTG)}
                  className={`px-3 py-1 rounded-md text-xs font-bold transition-all ${activeCurrency === Currency.HTG ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}
                >
                  G HTG
                </button>
              </div>
          ...
          {/* Bottom Navigation - Mobile & Tablet (under lg) */}
...
const NavLink: React.FC<{ to: string, icon: string, label: string }> = ({ to, icon, label }) => {
  ...
  return (
    <Link 
      to={to} 
      className={`flex items-center gap-3 py-4 px-6 transition-all border-l-4 ${isActive ? 'bg-blue-600/10 border-blue-500 text-blue-400' : 'border-transparent text-gray-400 hover:text-gray-100'}`}
    >
...
const MobileNavLink: React.FC<{ to: string, icon: string, label: string }> = ({ to, icon, label }) => {
  ...
  return (
    <Link 
      to={to} 
      className={`flex flex-col items-center justify-center flex-1 p-2 transition-all ${isActive ? 'text-blue-600' : 'text-gray-400'}`}
    >
